...
import { Shield, Settings, PlusCircle } from 'lucide-react';
  ...
  return (
    <nav className="sticky top-0 z-50 w-full glass-dark px-6 py-4 flex items-center justify-between border-b border-white/10">
      ...
      <div className="flex items-center space-x-5">
        <button className="hidden sm:flex p-2.5 text-slate-400 hover:text-white hover:bg-white/5 rounded-full transition-all">
          <Settings size={22} />
        </button>
        <div className="h-11 w-11 rounded-full border-2 border-emerald-500/50 p-0.5 overflow-hidden cursor-pointer hover:border-emerald-400 hover:scale-110 transition-all shadow-lg ring-4 ring-emerald-500/5">
